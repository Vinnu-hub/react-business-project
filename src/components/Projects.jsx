...
return()=> window.removeEventListener('resize',updateCardsToShow);
  ...
  const nextProject = () => {
    setCurrentIndex((prevIndex) => (prevIndex + 1) % projectsData.length);  // Fixed typo with prevIndex
  };
  
  // Function to go to the previous project
  const prevProject = () => {
    setCurrentIndex((prevIndex) => (prevIndex === 0 ? projectsData.length - 1 : prevIndex - 1));  // Fixed typo with prevIndex
  };
    ...
    <div className='container mx-auto py-4 pt-20 px-6 md:px-30 lg:px-32 w-full overflow-hidden' id='Projects'>
      ...
      <div className='overflow-hidden'>
        <div 
          ...
          style={{
            transform: `translateX(-${(currentIndex * 100) / cardsToShow}%)`,  // Corrected syntax for transform
          }}
        >
